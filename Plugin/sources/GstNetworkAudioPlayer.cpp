...
class GstNetworkAudioPlayerImpl : public GstPipelineHandler {
 ...
 public:
  ...
  virtual ~GstNetworkAudioPlayerImpl() { Close(); }

#define OPUS_ENC
  void _BuildPipeline() {
#ifdef FLAC_ENC
    core::string audiocaps =
        " caps=application/"
        "x-rtp,media=application,clock-rate=90000,encoding-name=X-GST,payload="
        "96 ";
    core::string audioStr =
        " rtpgstdepay ! audio/x-flac,rate=8000,channels=1! flacdec !  "
        "audioconvert ! audioresample  ";
#elif defined OPUS_ENC

    std::string audiocaps =
        "caps=application/"
        "x-rtp,media=(string)audio,clock-rate=48000,encoding-name=OPUS,payload="
        "96,encoding-params=2 ";
    char buffer[128];
    ...
    std::string audiocaps =
        "caps=application/"
        "x-rtp,media=(string)audio,clock-rate=32000,encoding-name=VORBIS,"
        "payload=96,encoding-params=2 ";
    char buffer[128];
    sprintf(buffer, "%d", m_sampleRate);
    ...
    std::
        string
            audioStr =
                " rtpvorbisdepay ! vorbisdec ! audioconvert   ! audio/x-raw, "
                "rate=";  //! audioresample quality=10
    audioStr += buffer;
    audioStr += " ";
#elif defined SPEEX_ENC
    core::string audiocaps =
        "caps=application/"
        "x-rtp,media=(string)audio,clock-rate=(int)22000,encoding-name=(string)"
        "SPEEX ";
    core::string audioStr =
    ...
    core::string audiocaps =
        "caps=application/"
        "x-rtp,media=(string)audio,clock-rate=(int)8000,encoding-name=(string)"
        "AMR,encoding-params=(string)1,octet-align=(string)1 ";
    ...
    if (m_rtcp) {
      m_pipeLineString =
          ...
          // audio rtcp
          "rtpbin.send_rtcp_src_0 ! myudpsink name=audioRtcpSink sync=false "
          "async=false ";
    } else {
      m_pipeLineString =
      ...
      if (m_customAudioInterface) {
        m_pipeLineString +=
            "! audioresample ! audio/x-raw,format=F32LE,rate=";  //! audioresample
                                                                 //! quality=10
        m_pipeLineString += buffer;
        m_pipeLineString +=
            " ! appsink name=audioSink sync=false  emit-signals=false";
      } else {
...
#define SET_SRC(name, p)                                                       \
  m_##name = GST_MyUDPSrc(gst_bin_get_by_name(GST_BIN(GetPipeline()), #name)); \
  if (m_##name) {                                                              \
    m_##name->SetPort(p);                                                      \
  }
#define SET_SINK(name, p)                                                \
  ...
  if (m_##name) {                                                        \
    ...
    if (m_audioSrc) g_object_set(m_audioSrc, "port", m_audioPort, 0);
  ...
  void SetIPAddress(const std::string& ip, uint audioPort, uint clockPort,
                    bool rtcp) {
    m_ipAddr = ip;
  ...
  bool CreateStream() {
    ...
    if (err) {
      LogMessage("NetworkAudioPlayer:CreateStream() - Pipeline error:" +
    ...
    LogMessage("NetworkAudioPlayer:CreateStream() - Pipeline created",
               ELL_INFO);
  ...
  virtual bool IsUsingCustomAudioInterface() { return m_customAudioInterface; }
  int GetPort() {
    if (m_audioSrc) {
      int port;
      g_object_get(m_audioSrc, "port", &port, 0);
  ...
  int GetSampleRate() { return m_audioHandler.SampleRate(); }

  bool GrabFrame() {
    if (!m_customAudioInterface) return false;
    return m_audioHandler.GrabFrame();
  }
  int GetFrameSize() {
    if (!m_customAudioInterface) return 0;
...
GstNetworkAudioPlayer::~GstNetworkAudioPlayer() { delete m_impl; }
void GstNetworkAudioPlayer::SetIPAddress(const std::string& ip, uint audioPort,
                                         uint clockPort, bool rtcp) {
  m_impl->SetIPAddress(ip, audioPort, clockPort, rtcp);
}
bool GstNetworkAudioPlayer::CreateStream() { return m_impl->CreateStream(); }
uint GstNetworkAudioPlayer::GetAudioPort() { return m_impl->GetAudioPort(); }

void GstNetworkAudioPlayer::SetVolume(float vol) { m_impl->SetVolume(vol); }
bool GstNetworkAudioPlayer::IsStream() { return m_impl->IsStream(); }
GstPipelineHandler* GstNetworkAudioPlayer::GetPipeline() { return m_impl; }

void GstNetworkAudioPlayer::Play() { m_impl->Play(); }
void GstNetworkAudioPlayer::Stop() { m_impl->Stop(); }
void GstNetworkAudioPlayer::Pause() { m_impl->Pause(); }
bool GstNetworkAudioPlayer::IsLoaded() { return m_impl->IsLoaded(); }
bool GstNetworkAudioPlayer::IsPlaying() { return m_impl->IsPlaying(); }
void GstNetworkAudioPlayer::Close() { m_impl->Close(); }

void GstNetworkAudioPlayer::SetSampleRate(int Rate) {
  m_impl->SetSampleRate(Rate);
}
int GstNetworkAudioPlayer::GetSampleRate() { return m_impl->GetSampleRate(); }
...
bool GstNetworkAudioPlayer::GrabFrame(int i) { return m_impl->GrabFrame(); }
int GstNetworkAudioPlayer::GetFrameSize() { return m_impl->GetFrameSize(); }
bool GstNetworkAudioPlayer::CopyAudioFrame(float* output) {
  return m_impl->CopyAudioFrame(output);
}
int GstNetworkAudioPlayer::ChannelsCount() { return m_impl->ChannelsCount(); }
int GstNetworkAudioPlayer::GetPort(int i) { return m_impl->GetPort(); }
