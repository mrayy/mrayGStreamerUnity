...
class CustomAudioGrabberImpl : public GstPipelineHandler {
 ...
 public:
  ...
  virtual ~CustomAudioGrabberImpl() { Close(); }
  ...
  std::string BuildPipeline() {
    ...
    if (samplingrate > 0 && channels > 0) {
      ss << "  ! audioresample ! "
            "audio/x-raw,endianness=1234,signed=true,width=16,depth=16,rate="
         << samplingrate << ",channels="
         << channels;  // << " ";
                       //" ! audiochebband mode=band-pass lower-frequency=1000
                       //upper-frequency=6000 poles=4 "
    }
    ss << " ! audiorate ! audioconvert !audio/x-raw,format=F32LE ";  // , rate =
                                                                     // " <<
                                                                     // samplingrate;
  ...
  bool Start() {
    ...
    LogMessage("CustomAudioGrabber::Starting with pipeline: " + pipline,
               ELL_INFO);
    if (err) {
      LogMessage(
          "CustomAudioGrabber: Pipeline error: " + std::string(err->message),
          ELL_WARNING);
      gst_object_unref(p);
      p = 0;
    }
    ...
    // g_signal_connect(m_audioSink, "new-sample", G_CALLBACK(new_buffer),
    // this); attach videosink callbacks
  ...
  void Restart() { GstPipelineHandler::SetPosition(0); }
  void Close() {
    audiohandler.Close();
    GstPipelineHandler::Close();
  }
  bool IsStarted() { return GstPipelineHandler::IsPlaying(); }

  uint GetSamplingRate() { return samplingrate; }
  uint GetChannelsCount() { return channels; }
  ...
  uint GetAudioFrameSize() { return frame.size(); }
...
CustomAudioGrabber::~CustomAudioGrabber() { delete m_impl; }

GstPipelineHandler* CustomAudioGrabber::GetPipelineHandler() { return m_impl; }

void CustomAudioGrabber::Init(const std::string& pipeline, int channels,
                              int samplingrate) {
  m_impl->Init(pipeline, channels, samplingrate);
}

bool CustomAudioGrabber::Start() { return m_impl->Start(); }
void CustomAudioGrabber::Pause() { m_impl->Pause(); }
void CustomAudioGrabber::Resume() { m_impl->SetPaused(false); }
void CustomAudioGrabber::Restart() { m_impl->Restart(); }
void CustomAudioGrabber::Close() { m_impl->Close(); }
bool CustomAudioGrabber::IsStarted() { return m_impl->IsStarted(); }

uint CustomAudioGrabber::GetSamplingRate() { return m_impl->GetSamplingRate(); }
uint CustomAudioGrabber::GetChannelsCount() {
  return m_impl->GetChannelsCount();
}

bool CustomAudioGrabber::GrabFrame() { return m_impl->GrabFrame(); }
float* CustomAudioGrabber::GetAudioFrame() { return m_impl->GetAudioFrame(); }
uint CustomAudioGrabber::GetAudioFrameSize() {
  return m_impl->GetAudioFrameSize();
}
void CustomAudioGrabber::SetVolume(float vol) { m_impl->SetVolume(vol); }
