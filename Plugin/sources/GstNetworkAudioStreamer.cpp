...
#define OPUS_ENC
        ...
        std::string audioStr =
            "directsoundsrc buffer-time=100  ! "
            "audio/"
            "x-raw,endianness=1234,signed=true,width=16,depth=16,channels=" +
            std::string(buffer) +
            "  ! audioconvert ! audioresample";  // !tee name = t ";

        for (int i = 0; i < m_clients.size() && i < 1; ++i) {
            sprintf(buffer, "%d", i);
            // audioStr+= " t. ! queue ! volume  volume=1
            // name=vol"+std::string(buffer);
            audioStr +=
                " ! volume  volume=1 name=vol" + std::string(buffer) + " ! ";
#ifdef VORBIS_ENC
            audioStr +=
                "vorbisenc quality=1 ! rtpvorbispay config-interval=3  ";
#elif defined OPUS_ENC
            audioStr +=
                "opusenc complexity=10 bitrate-type=vbr frame-size=5 ! "
                "rtpopuspay  ";
#endif
            audioStr += "! udpsink name=audioSink" + std::string(buffer) +
                        " sync=false";
    ...
    void _UpdatePorts() {
        ...
        //#define SET_SINK(target,name,p)
        //target=GST_MyUDPSink(gst_bin_get_by_name(GST_BIN(GetPipeline()),
        //name)); if(target){target->SetPort(p.address,p.audioPort);}

        char buffer[128];
        for (int i = 0; i < m_clients.size(); ++i) {
            sprintf(buffer, "audioSink%d", i);
            m_clients[i].audioSink =
                gst_bin_get_by_name(GST_BIN(GetPipeline()), buffer);
            if (m_clients[i].audioSink)
                g_object_set(m_clients[i].audioSink, "port",
                             m_clients[i].audioPort, "host",
                             m_clients[i].address.c_str(), 0);
    ...
    bool _CreateStream(bool isInternal) {
        ...
        if (err) {
            LogMessage("GstAudioNetworkStreamer: Pipeline error: " +
    ...
    bool CreateStream() { return _CreateStream(false); }
...
GstNetworkAudioStreamer::~GstNetworkAudioStreamer() { delete m_impl; }
void GstNetworkAudioStreamer::Stream() { m_impl->Stream(); }
void GstNetworkAudioStreamer::Stop() { m_impl->Stop(); }
GstPipelineHandler* GstNetworkAudioStreamer::GetPipeline() { return m_impl; }

void GstNetworkAudioStreamer::AddClient(const std::string& addr, uint port,
                                        uint clockPort, bool rtcp) {
    m_impl->AddClient(addr, port);
}
void GstNetworkAudioStreamer::RemoveClient(int i) { m_impl->RemoveClient(i); }
void GstNetworkAudioStreamer::ClearClients() { m_impl->ClearClients(); }
...
bool GstNetworkAudioStreamer::CreateStream() { return m_impl->CreateStream(); }

void GstNetworkAudioStreamer::Close() { m_impl->Close(); }
void GstNetworkAudioStreamer::SetChannels(int channels) {
    m_impl->SetChannels(channels);
}

bool GstNetworkAudioStreamer::IsStreaming() { return m_impl->IsStreaming(); }

void GstNetworkAudioStreamer::SetPaused(bool paused) {
    m_impl->SetPaused(paused);
}

bool GstNetworkAudioStreamer::IsPaused() { return !m_impl->IsPlaying(); }
