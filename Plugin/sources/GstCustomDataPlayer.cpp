...
class GstCustomDataPlayerImpl : public GstPipelineHandler {
 ...
 public:
  ...
  GstClockTime getTimeStamp() {
    ...
    GstClockTime now =
        gst_clock_get_time(clock) - gst_element_get_base_time(GetPipeline());
  ...
  void _BuildPipeline() {
    std::stringstream ss;
    {
      ss << "udpsrc name=src port=" << m_port << " ! application/x-"
         << m_dataType << ",media=application,payload=" << m_payload
         << " ! rtpgstdepay !  appsink name=datasink sync=false";
    }
  ...
  bool CreateStream() {
    ...
    g_object_set(G_OBJECT(m_sink), "emit-signals", FALSE, "sync", false,
                 (void*)NULL);
...
#if GST_VERSION_MAJOR == 0
    shared_ptr<GstBuffer> buffer(gst_app_sink_pull_buffer(GST_APP_SINK(elt)),
                                 &gst_buffer_unref);
#else
    std::shared_ptr<GstSample> buffer(
        gst_app_sink_pull_sample(GST_APP_SINK(elt)), &gst_sample_unref);
#endif
...
#if GST_VERSION_MAJOR == 0
    shared_ptr<GstBuffer> buffer(gst_app_sink_pull_preroll(GST_APP_SINK(elt)),
                                 &gst_buffer_unref);
#else
    std::shared_ptr<GstSample> buffer(
        gst_app_sink_pull_preroll(GST_APP_SINK(elt)), &gst_sample_unref);
#endif
...
GstPipelineHandler* GstCustomDataPlayer::GetPipeline() { return m_impl; }

GstCustomDataPlayer::GstCustomDataPlayer() {
  m_impl = new GstCustomDataPlayerImpl();
}

GstCustomDataPlayer::~GstCustomDataPlayer() { delete m_impl; }

void GstCustomDataPlayer::SetApplicationDataType(const std::string& dataType,
                                                 bool autotimestamp,
                                                 int payload) {
  m_impl->SetApplicationDataType(dataType, autotimestamp, payload);
}

bool GstCustomDataPlayer::CreateStream() { return m_impl->CreateStream(); }

void GstCustomDataPlayer::SetVolume(float vol) { m_impl->SetVolume(vol); }
bool GstCustomDataPlayer::IsStream() { return m_impl->IsStream(); }

void GstCustomDataPlayer::Play() { m_impl->Play(); }
void GstCustomDataPlayer::Stop() { m_impl->Stop(); }
void GstCustomDataPlayer::Pause() { m_impl->Pause(); }
bool GstCustomDataPlayer::IsLoaded() { return m_impl->IsLoaded(); }
bool GstCustomDataPlayer::IsPlaying() { return m_impl->IsPlaying(); }
void GstCustomDataPlayer::Close() { m_impl->Close(); }

void GstCustomDataPlayer::SetPort(int i) { m_impl->SetPort(i); }
int GstCustomDataPlayer::GetPort(int i) { return m_impl->GetPort(); }

bool GstCustomDataPlayer::GrabFrame(int i) { return m_impl->GrabFrame(); }
bool GstCustomDataPlayer::HasNewFrame() { return m_impl->HasNewFrame(); }
ulong GstCustomDataPlayer::GetBufferID() { return m_impl->GetBufferID(); }

int GstCustomDataPlayer::GetDataLength() { return m_impl->GetDataLength(); }
