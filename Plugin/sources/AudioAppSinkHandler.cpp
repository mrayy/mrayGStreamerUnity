...
GstFlowReturn AudioAppSinkHandler::process_sample(
    std::shared_ptr<GstSample> sample, bool preroll) {
  if (m_closed) return GST_FLOW_ERROR;
...
int AudioAppSinkHandler::GetFrameSize() {
  OS::ScopedLock m(m_mutex);
  if (!m_grabbedFrame) return 0;
  return m_grabbedFrame->dataLength;
}

void AudioAppSinkHandler::CopyAudioFrame(float* output) {
  OS::ScopedLock m(m_mutex);
  if (!m_grabbedFrame || !output) return;
  memcpy(output, m_grabbedFrame->data,
         m_grabbedFrame->dataLength * sizeof(float));
