...
class NetworkAudioGrabberData : public IPipelineListener {
   public:
    ...
    virtual void OnPipelineClosed(GstPipelineHandler* p) { m_audioSrc = 0; }
...
void NetworkAudioGrabber::Init(uint audioPort, int channels, int samplingrate) {
    ...
    std::string audioStr =
        "  queue ! rtpopusdepay ! opusdec plc=true ! audioconvert ";

    std::stringstream pipeline;
    pipeline << "udpsrc name=audioSrc port=" << (audioPort) << " " << audiocaps
             << "!" << audioStr;  //
...
uint NetworkAudioGrabber::GetAudioPort() { return m_data->GetAudioPort(); }
