...
static GstFlowReturn gst_MyUDPSink_render(GstBaseSink *sink, GstBuffer *buffer);
...
static void gst_MyUDPSink_class_init(GstMyUDPSinkClass *klass) {
    ...
    g_object_class_install_property(
        ...
        g_param_spec_int(
            "port", "port", "The port to send the packets to", 0, 65535,
            UDP_DEFAULT_PORT,
            (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS)));
    ...
    gst_element_class_set_details_simple(
        gstelement_class, "MyUDPSink class", "Sink", "Custom UDP Sink",
...
static void gst_MyUDPSink_init(GstMyUDPSink *sink) {
...
static GstFlowReturn gst_MyUDPSink_render(GstBaseSink *bsink,
                                          GstBuffer *buffer) {
    GstMyUDPSink *sink;
    GError *err = NULL;
    GstMapInfo map;
    ...
    if (!sink->m_client || !sink->m_client->IsConnected()) return GST_FLOW_OK;
    ...
    for (int i = 0; i < n_mem; i++) {
            ...
            if (map.size > UDP_MAX_SIZE) {
                    GST_WARNING("Attempting to send a UDP packet larger than
maximum " "size (%" G_GSIZE_FORMAT " > %d)", map.size, UDP_MAX_SIZE);
            }
            unsigned int len = 0;
            sink->m_client->SendTo(0, (const char*)map.data, map.size,&len);
...
void GstMyUDPSink::SetPort(const std::string &host, guint16 p) {
    if (this->host == host && port == p) return;
    this->host = host;
    port = p;
...
static void gst_MyUDPSink_set_property(GObject *object, guint prop_id,
    ...
    switch (prop_id) {
        case PROP_HOST: {
            if (sink->host == g_value_get_string(value)) break;
            sink->host = g_value_get_string(value);
            update = true;
            break;
        }
        case PROP_PORT:
            if (sink->port == g_value_get_int(value)) break;
            update = true;
...
static gboolean gst_MyUDPSink_start(GstBaseSink *bsink) {
    ...
    if (sink->m_client) {
        ...
        if (sink->m_client->Connect(
                network::NetAddress(sink->host, sink->port))) {
            GST_DEBUG_OBJECT(sink, "UDP Socket connected to (%s:%d)",
                             sink->host.c_str(), sink->port);
        } else {
            GST_DEBUG_OBJECT(sink, "Failed to connect UDP Socket to (%s:%d)",
                             sink->host.c_str(), sink->port);
            // return FALSE;
        }
    }
...
void gst_MyUDPSink_clear(GstMyUDPSink *sink) {}
...
gboolean _GstMyUDPSinkClass::plugin_init(GstPlugin *plugin) {
