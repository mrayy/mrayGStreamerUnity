   ...
   public:
    ...
    std::string BuildGStr() {
        ...
        if (m_grabber) {
            ...
            // std::stringConverter::toString(m_fps) + " "; 	videoStr += " !
            // queue
    ...
    bool BuildString() {
        ...
        if ((width < m_grabber->GetFrameSize().x ||
             height < m_grabber->GetFrameSize().y)) {
    ...
    void SetEncoderSettings(const std::string& param,
    ...
    GstFlowReturn NeedBuffer(GstMySrc* sink, GstBuffer** buffer, int index) {
        if (!GetPipeline() || !IsStreaming()) return GST_FLOW_ERROR;
        if (!m_grabber) {
            LogMessage(
                std::string(
                    "No video grabber is assigned to CustomVideoStreamer"),
                ELL_WARNING);
...
#define SET_SINK(name, p)                                                  \
    ...
    if (m_##name) {                                                        \
...

        {
            ...
            if (m_videoSrc.videoSrc) {
                ...
                // 640 * 480 * 3);
                // gst_base_src_set_live(GST_BASE_SRC(m_videoSrc.videoSrc),
                // true);
                ...
                // gst_app_src_set_max_bytes(m_videoSrc.videoSrc, 640 * 480 *
                // 3); gst_app_src_set_emit_signals(m_videoSrc.videoSrc, false);
                // gst_base_src_set_do_timestamp(GST_BASE_SRC(m_videoSrc.videoSrc),
                // true);
                ...
                // 					"format",
                // GST_FORMAT_TIME, "is-live", TRUE, NULL);

                // 				g_signal_connect(m_videoSrc.videoSrc,
                // "need-data", G_CALLBACK(start_feed), &m_videoSrc.srcCB);
                // 				g_signal_connect(m_videoSrc.videoSrc,
                // "enough-data", G_CALLBACK(stop_feed), &m_videoSrc.srcCB);
        ...
        g_object_set(m_videoSink, "port", m_videoPort, "host", m_ipAddr.c_str(),
                     0);

        // SET_SINK(videoSink, m_videoPort);
    }

    void BindPorts(const std::string& addr, int videoPort, uint clockPort,
                   bool rtcp) {
    ...
    virtual void Close() {
        ...
        //	gst_element_send_event(GST_ELEMENT(m_videoSrc.videoSrc),
        // gst_event_new_eos());
...
GstNetworkVideoStreamer::~GstNetworkVideoStreamer() { delete m_impl; }
void GstNetworkVideoStreamer::Stream() { m_impl->Stream(); }
void GstNetworkVideoStreamer::Stop() { m_impl->Stop(); }

void GstNetworkVideoStreamer::BindPorts(const std::string& addr, uint videoPort,
                                        uint clockPort, bool rtcp) {
    m_impl->BindPorts(addr, videoPort, clockPort, rtcp);
}

bool GstNetworkVideoStreamer::CreateStream() { return m_impl->CreateStream(); }

void GstNetworkVideoStreamer::Close() { m_impl->Close(); }
bool GstNetworkVideoStreamer::IsStreaming() { return m_impl->IsStreaming(); }
...
bool GstNetworkVideoStreamer::IsPaused() { return !m_impl->IsPlaying(); }
GstPipelineHandler* GstNetworkVideoStreamer::GetPipeline() { return m_impl; }
