...
bool GstPipelineHandler::CreatePipeline(bool isMasterClock,
    ...
    if (gst_element_get_state(GST_ELEMENT(m_data->gstPipeline), NULL, NULL,
        ...
        LogMessage(
            "GstPipelineHandler::Play(): unable to get pipeline ready status",
            ELL_WARNING);
        return false;
    }
    ...
    if (gst_element_set_state(GST_ELEMENT(m_data->gstPipeline),
        ...
        LogMessage("GstPipelineHandler::Play(): unable to pause pipeline",
                   ELL_WARNING);
        return false;
    }
...
bool GstPipelineHandler::SetPosition(signed long pos) {
    if (!gst_element_seek(m_data->gstPipeline, 1.0, GST_FORMAT_TIME,
                          GST_SEEK_FLAG_FLUSH, GST_SEEK_TYPE_SET, pos * 1000,
...
bool GstPipelineHandler::HandleMessage(GstBus* bus, GstMessage* msg) {
    ...
    switch (GST_MESSAGE_TYPE(msg)) {
        case GST_MESSAGE_EOS:
            LogMessage(std::string("GstPipelineHandler::HandleMessage(): "
        ...
        case GST_MESSAGE_DURATION: {
            ...
            gst_element_query_duration(m_data->gstPipeline, &format,
                                       &durationNanos);
        } break;
#else
        ...
        case GST_MESSAGE_WARNING: {
            ...
            LogMessage(
                std::string(
                    "GstPipelineHandler::HandleMessage(): warning in module ") +
        ...
        case GST_MESSAGE_ERROR: {
            ...
            LogMessage(
                std::string(
                    "GstPipelineHandler::HandleMessage(): error in module ") +
                    name + std::string("  reported: ") + err->message +
                    std::string(" - ") + debug,
                ELL_WARNING);
...
void GstPipelineHandler::SetClockBaseTime(ulong baseTime) {
    ...
    if (m_data->gstPipeline) {
        GstClock* clock =
            gst_pipeline_get_clock(GST_PIPELINE(m_data->gstPipeline));
