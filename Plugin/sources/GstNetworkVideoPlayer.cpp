   ...
   public:
    ...
    void _BuildPipelineH264() {
        ...
        m_pipeLineString =
            ...
            //	"videoconvert ! video/x-raw,format=RGB  !" // Very
            // slow!!
...
#define SET_SINK(name, p)                                                  \
    ...
    if (m_##name) {                                                        \
        ...
        g_object_set(m_videoSrc, "port", m_videoPort, "host", m_ipAddr.c_str(),
                     0);
    }

    void SetIPAddress(const std::string& ip, uint videoPort, uint clockPort,
                      bool rtcp) {
        m_ipAddr = ip;
    ...
    bool CreateStream() {
        ...
        if (err) {
            LogMessage("GstNetworkVideoPlayer: Pipeline error: " +
        ...
        m_videoSink =
            GST_APP_SINK(gst_bin_get_by_name(GST_BIN(p), "videoSink"));

        m_videoHandler.SetSink(m_videoSink);
        g_signal_connect(m_videoSink, "new-sample", G_CALLBACK(new_buffer),
                         this);
        ...
        gstCallbacks.new_buffer =
            &VideoAppSinkHandler::on_new_buffer_from_source;
#else
        gstCallbacks.new_sample =
            &VideoAppSinkHandler::on_new_buffer_from_source;
#endif
        ...
        // TRUE);
        // 		gst_base_sink_set_sync(GST_BASE_SINK(m_videoSink),
        // false); 		gst_app_sink_set_drop(GST_APP_SINK(m_videoSink),
        // TRUE);
        //  		gst_app_sink_set_max_buffers(GST_APP_SINK(m_videoSink),
        //  8);
...
GstNetworkVideoPlayer::~GstNetworkVideoPlayer() { delete m_impl; }
void GstNetworkVideoPlayer::SetIPAddress(const std::string& ip, uint videoPort,
                                         uint clockPort, bool rtcp) {
    m_impl->SetIPAddress(ip, videoPort, clockPort, rtcp);
}
bool GstNetworkVideoPlayer::CreateStream() { return m_impl->CreateStream(); }
GstPipelineHandler* GstNetworkVideoPlayer::GetPipeline() { return m_impl; }

void GstNetworkVideoPlayer::SetVolume(float vol) { m_impl->SetVolume(vol); }
bool GstNetworkVideoPlayer::IsStream() { return m_impl->IsStream(); }

void GstNetworkVideoPlayer::Play() { m_impl->Play(); }
void GstNetworkVideoPlayer::Stop() { m_impl->Stop(); }
void GstNetworkVideoPlayer::Pause() { m_impl->Pause(); }
bool GstNetworkVideoPlayer::IsLoaded() { return m_impl->IsLoaded(); }
bool GstNetworkVideoPlayer::IsPlaying() { return m_impl->IsPlaying(); }
void GstNetworkVideoPlayer::Close() { m_impl->Close(); }
...
bool GstNetworkVideoPlayer::GrabFrame(int i) { return m_impl->GrabFrame(); }

bool GstNetworkVideoPlayer::HasNewFrame() { return m_impl->HasNewFrame(); }

ulong GstNetworkVideoPlayer::GetBufferID() { return m_impl->GetBufferID(); }
...
int GstNetworkVideoPlayer::GetPort(int i) { return m_impl->GetPort(i); }
