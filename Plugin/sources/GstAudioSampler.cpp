...
class GstAudioSamplerImpl : public GstPipelineHandler {
 ...
 public:
  ...
  GstFlowReturn NeedBuffer(GstBuffer** buffer) {
    ...
    if (!m_audioGrabber) {
      LogMessage(
          "GstAudioSampler::NeedBuffer() -No audio grabber is assigned to "
          "GstAudioSampler",
          ELL_WARNING);
  ...
  static gboolean read_data(GstAudioSamplerImpl* d) {
    ...
    if (d->NeedBuffer(&buffer) == GST_FLOW_OK) {
      ...
      if (ret != GST_FLOW_OK) {
        LogMessage(
            "GstAudioSampler::read_data() - Failed to push data to AppSrc ",
            ELL_WARNING);
  ...
  void _BuildPipeline() {
    ...
    ss << "appsrc name=src is-live=true block=true stream-type=0 format=3 "
          "do-timestamp=true ";

    ss << " ! audio/x-raw,format=F32LE,rate="
       << (m_audioGrabber->GetSamplingRate())
       << ",channels=" << (m_audioGrabber->GetChannelsCount())
       << " ! audioconvert ";
    // 		ss << " ! audioparse format=F32LE width=32 depth=32 signed=true rate="
    // << (m_audioGrabber->GetSamplingRate()) << 			" channels=" <<
    // (m_audioGrabber->GetChannelsCount()) << "   ";

    if (m_sampleRate != -1 &&
        m_sampleRate != m_audioGrabber->GetSamplingRate()) {
  ...
  bool CreateStream() {
    ...
    if (err) {
      LogMessage("GstAudioSampler:CreateStream() - Pipeline error:" +
  ...
  bool GrabFrame() { return m_audioHandler.GrabFrame(); }
  int GetFrameSize() { return m_audioHandler.GetFrameSize(); }
  bool CopyAudioFrame(float* output) {
    m_audioHandler.CopyAudioFrame(output);
    return true;
  }
  int ChannelsCount() { return m_audioHandler.ChannelsCount(); }
};

GstAudioSampler::GstAudioSampler() { m_impl = new GstAudioSamplerImpl(); }

GstAudioSampler::~GstAudioSampler() { delete m_impl; }
void GstAudioSampler::Init(int audioInterface, int samplingRate) {
  m_impl->Init(audioInterface, samplingRate);
}
bool GstAudioSampler::CreateStream() { return m_impl->CreateStream(); }
void GstAudioSampler::SetVolume(float vol) { m_impl->SetVolume(vol); }
bool GstAudioSampler::IsStream() { return m_impl->IsStream(); }
GstPipelineHandler* GstAudioSampler::GetPipeline() { return m_impl; }

void GstAudioSampler::Play() { m_impl->Play(); }
void GstAudioSampler::Stop() { m_impl->Stop(); }
void GstAudioSampler::Pause() { m_impl->Pause(); }
bool GstAudioSampler::IsLoaded() { return m_impl->IsLoaded(); }
bool GstAudioSampler::IsPlaying() { return m_impl->IsPlaying(); }
void GstAudioSampler::Close() { m_impl->Close(); }
void GstAudioSampler::SetAudioGrabber(IAudioGrabber* g) {
  m_impl->SetAudioGrabber(g);
}

void GstAudioSampler::SetSampleRate(int Rate) { m_impl->SetSampleRate(Rate); }
int GstAudioSampler::GetSampleRate() { return m_impl->GetSampleRate(); }

bool GstAudioSampler::GrabFrame() { return m_impl->GrabFrame(); }
int GstAudioSampler::GetFrameSize() { return m_impl->GetFrameSize(); }
bool GstAudioSampler::CopyAudioFrame(float* output) {
  return m_impl->CopyAudioFrame(output);
}
int GstAudioSampler::ChannelsCount() { return m_impl->ChannelsCount(); }
