...
class GstNetworkMultipleVideoPlayerImpl : public GstPipelineHandler,
                                          IPipelineListener {
 ...
 public:
  ...
  void SetDecoderType(std::string type) { m_encoderType = type; }
  ...
  bool GetRecordStarted() {
    if (recordFileName == "") return false;

    for (int i = 0; i < m_videoHandler.size(); ++i)
      if (m_videoHandler[i]->handler.GetSamplesCount() > 0) return true;
    return false;
  }
  ...
  std::string _BuildPipeline(int i) {
    std::stringstream ss;
    std::string videoStr;
    ss << "udpsrc name=videoSrc" << i
       << " port=" << m_videoHandler[i]->videoPort
       << " !"
    ...
    if (m_rtcp) {
      ss << "! rtpbin.recv_rtp_sink_" << i << " rtpbin. ";
    } else {
    ...
    if (m_encoderType == "H264") {
      ...
      ss << " ! mylistener name=rtplistener" << i
         << "  ! rtph264depay ! mylistener name=postdepay" << i;
      ...
      if (recordFileName == "")
        ...
        if (limitRecordFramerate > 0) {
          ...
          ss << " t. ! queue ! videoscale ! videorate ! video/x-raw,framerate="
             << limitRecordFramerate
             << "/1 ! videoconvert ! x264enc pass=qual ! mp4mux name=muxer" << i
             << " ! filesink name=fs" << i << " location=\"" << recordFileName
             << "\" sync=false ";
          ss << " t. ! queue ! ";
        } else {
          ss << " ! tee name=t t. ! queue ! h264parse ! mp4mux name=muxer" << i
             << " ! filesink name=fs" << i << " location=\"" << recordFileName
             << "\" sync=false ";
      ...
      ss << ",media=video,encoding-name=JPEG,payload=96 ! queue ! rtpjpegdepay "
            "! jpegdec ! videoconvert !";
    } else if (m_encoderType == "VP8") {
      ss << "  ! rtpvp8depay ! vp8dec ! videoconvert !";
    } else if (m_encoderType == "VP9") {
      ss << "  ! mylistener name=rtplistener" << i
         << "  ! rtpvp9depay ! mylistener name=postdepay" << i
         << " ! vp9dec  threads=3  !";
    } else
      return "";
    //"rtpvp8depay ! vp8dec ! "
    ...
    if (m_rtcp) {
      ss << " appsink name=videoSink" << i <<
         ...
         << " "
            "rtpbin.send_rtcp_src_"
         << i << " ! udpsink name=videoRtcpSink" << i
         << " sync=false async=false ";
    } else {
      ss << " mylistener name=preappsrc" << i << " !  appsink name=videoSink"
         << i << " sync=false  emit-signals=true ";  // fpsdisplaysink ";// "
    }
...
#define SET_SRC(idx, name, p)                                          \
  m_videoHandler[idx].videoSrc =                                       \
      GST_MyUDPSrc(gst_bin_get_by_name(GST_BIN(GetPipeline()), name)); \
  if (m_videoHandler[idx].videoSrc) {                                  \
    m_videoHandler[idx].videoSrc->SetPort(p);                          \
  }
#define SET_SINK(idx, name, p)                                          \
  m_videoHandler[idx].videoSink =                                       \
      GST_MyUDPSink(gst_bin_get_by_name(GST_BIN(GetPipeline()), name)); \
  if (m_videoHandler[idx].videoSink) {                                  \
    m_videoHandler[idx].videoSink->SetPort(m_ipAddr, p);                \
  }

    for (int i = 0; i < m_playersCount; ++i) {
      std::stringstream ss;
      ss << "videoSrc" << i;
      m_videoHandler[i]->videoSrc =
          gst_bin_get_by_name(GST_BIN(GetPipeline()), ss.str().c_str());
      if (m_videoHandler[i]->videoPort != 0)
        g_object_set(m_videoHandler[i]->videoSrc, "port",
                     m_videoHandler[i]->videoPort, 0);
      //	SET_SRC(i, ss.str().c_str(), m_videoHandler[i]->videoPort);
      // 			SET_SINK(i, videoRtcpSink,
      // (m_videoHandler[i]->videoPort + 1)); 			SET_SRC(i, videoRtcpSrc,
      // (m_videoHandler[i]->videoPort + 2));
    }
  }

  void SetIPAddress(const std::string& ip, uint videoPort, int count,
                    uint clockPort, bool rtcp) {
    ...
    for (int i = 0; i < m_playersCount; ++i) {
      if (m_videoHandler[i] == 0) m_videoHandler[i] = new VideoHandler();
      if (m_baseVideoSrc != 0)
  ...
  bool CreateStream() {
    ...
    if (err) {
      LogMessage("GstNetworkMultipleVideoPlayer: Pipeline error: " +
    ...
    for (int i = 0; i < m_playersCount; ++i) {
      {
        std::stringstream ss;
        ss << "videoSink" << i;
        m_videoHandler[i]->videoSink =
            GST_APP_SINK(gst_bin_get_by_name(GST_BIN(p), ss.str().c_str()));
      }
      {
        std::stringstream ss;
        ss << "rtplistener" << i;
        m_videoHandler[i]->rtpListener =
            GST_MyListener(gst_bin_get_by_name(GST_BIN(p), ss.str().c_str()));
      }
      {
        std::stringstream ss;
        ss << "postdepay" << i;
        m_videoHandler[i]->postdepayListener =
            GST_MyListener(gst_bin_get_by_name(GST_BIN(p), ss.str().c_str()));
      }
      {
        std::stringstream ss;
        ss << "preappsrc" << i;
        m_videoHandler[i]->preappsrcListener =
            GST_MyListener(gst_bin_get_by_name(GST_BIN(p), ss.str().c_str()));
      }
      {
        std::stringstream ss;
        ss << "muxer" << i;
        m_videoHandler[i]->Mp4Muxer =
            GST_ELEMENT(gst_bin_get_by_name(GST_BIN(p), ss.str().c_str()));
      }
      ...
      if (m_rtcp) {
        ...
        m_videoHandler[i]->videoRtcpSink =
            gst_bin_get_by_name(GST_BIN(p), rtcpSink.str().c_str());
        m_videoHandler[i]->videoRtcpSrc =
            gst_bin_get_by_name(GST_BIN(p), rtcpSrc.str().c_str());
      }

      m_videoHandler[i]->handler.SetSink(m_videoHandler[i]->videoSink);
      m_videoHandler[i]->handler.SetRTPListener(
          m_videoHandler[i]->rtpListener, m_videoHandler[i]->postdepayListener,
          m_videoHandler[i]->preappsrcListener);
      // m_videoHandler[i]->handler.SetSource(GST_MyUDPSrc(m_videoHandler[i]->videoSrc));
      g_signal_connect(m_videoHandler[i]->videoSink, "new-sample",
                       G_CALLBACK(new_buffer), this);
...
#endif
      gst_app_sink_set_callbacks(GST_APP_SINK(m_videoHandler[i]->videoSink),
                                 &gstCallbacks, &m_videoHandler[i]->handler,
                                 NULL);
      // gst_app_sink_set_emit_signals(GST_APP_SINK(m_videoSink), true);

      // 		gst_base_sink_set_async_enabled(GST_BASE_SINK(m_videoSink),
      // TRUE);
      gst_base_sink_set_sync(GST_BASE_SINK(m_videoHandler[i]->videoSink),
                             false);
      gst_app_sink_set_drop(GST_APP_SINK(m_videoHandler[i]->videoSink), TRUE);
      gst_app_sink_set_max_buffers(GST_APP_SINK(m_videoHandler[i]->videoSink),
                                   2);
      gst_base_sink_set_max_lateness(
          GST_BASE_SINK(m_videoHandler[i]->videoSink), 0);
      /*
      GstCaps* caps;
      ...
      // Tell the video appsink that it should not emit signals as the buffer
      retrieving is handled via callback methods g_object_set(m_videoSink,
      "emit-signals", false, "sync", false, "async", false, (void*)NULL);
      */
    }
  ...
  virtual void Close() {
    ...
    for (int i = 0; i < m_playersCount; ++i) {
      ...
      // 				gst_element_send_event(m_videoHandler[i]->videoSrc,
      // gst_event_new_eos());
      if (m_videoHandler[i]->Mp4Muxer != 0)
        gst_element_send_event(m_videoHandler[i]->Mp4Muxer,
                               gst_event_new_eos());
      // if (m_videoHandler[i]->videoSrc &&
      // m_videoHandler[i]->videoSrc->m_client)
  ...
  void SetImageFormat(video::EPixelFormat fmt) { m_targetFormat = fmt; }
  virtual video::EPixelFormat GetImageFormat(int i) {
    return m_videoHandler[i]->handler.getPixelsRef()->format;
  }
  int GetFramesCount() { return m_playersCount; }
  virtual bool GrabFrame(int i) {
    if (i >= m_playersCount) return false;
  ...
  virtual int GetPort(int i) { return GetVideoPort(i); }

  virtual void OnPipelineReady(GstPipelineHandler* p) {
    m_owner->__FIRE_OnPlayerReady(m_owner);
  }
  virtual void OnPipelinePlaying(GstPipelineHandler* p) {
    m_owner->__FIRE_OnPlayerStarted(m_owner);
  }
  virtual void OnPipelineStopped(GstPipelineHandler* p) {
    m_owner->__FIRE_OnPlayerStopped(m_owner);
  }
...
void GstNetworkMultipleVideoPlayer::SetIPAddress(const std::string& ip,
                                                 uint videoPort, uint count,
                                                 uint clockPort, bool rtcp) {
...
void GstNetworkMultipleVideoPlayer::SetRecordToFile(std::string filename,
                                                    int framerate) {
...
void GstNetworkMultipleVideoPlayer::AddIntermidateElement(
...
bool GstNetworkMultipleVideoPlayer::IsStream() { return m_impl->IsStream(); }

void GstNetworkMultipleVideoPlayer::Play() { m_impl->Play(); }
void GstNetworkMultipleVideoPlayer::Stop() { m_impl->Stop(); }
void GstNetworkMultipleVideoPlayer::Pause() { m_impl->Pause(); }
bool GstNetworkMultipleVideoPlayer::IsLoaded() { return m_impl->IsLoaded(); }
bool GstNetworkMultipleVideoPlayer::IsPlaying() { return m_impl->IsPlaying(); }
void GstNetworkMultipleVideoPlayer::Close() { m_impl->Close(); }
...
int GstNetworkMultipleVideoPlayer::GetPort(int i) { return m_impl->GetPort(i); }
