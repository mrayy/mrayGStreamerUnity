...
class GstCustomDataReceiverImpl : public GstPipelineHandler, IPipelineListener {
 ...
 public:
  ...
  void _BuildPipeline() {
    ...
    ss << " ! mylistener name=rtplistener ! rtpgstdepay ! mylistener "
          "name=postdepay ! appsink name=\"dataSink\" ";
  ...
  void _UpdatePorts() {
    ...
    if (m_dataPort != 0) g_object_set(m_dataSrc, "port", m_dataPort, 0);
  }

  void SetIPAddress(const std::string& ip, uint dataPort, uint clockPort,
                    bool rtcp) {
    m_ipAddr = ip;
  ...
  bool CreateStream() {
    ...
    if (err) {
      LogMessage(
          "GstCustomDataReceiver: Pipeline error: " + std::string(err->message),
          ELL_WARNING);
    }
    if (!p) return false;
    SetPipeline(p);
...
#endif
    gst_app_sink_set_callbacks(GST_APP_SINK(m_dataSink), &gstCallbacks,
                               &m_dataHandler, NULL);
  ...
  int GetFramesCount() { return 1; }
  virtual bool GrabFrame() { return m_dataHandler.GrabFrame(); }
  virtual bool HasNewFrame() { return m_dataHandler.isFrameNew(); }
  virtual ulong GetBufferID() { return m_dataHandler.GetFrameID(); }
...
GstCustomDataReceiver::~GstCustomDataReceiver() { delete m_impl; }
void GstCustomDataReceiver::SetIPAddress(const std::string& ip, uint videoPort,
                                         uint count, uint clockPort,
                                         bool rtcp) {
  m_impl->SetIPAddress(ip, videoPort, count, clockPort, rtcp);
}
bool GstCustomDataReceiver::CreateStream() { return m_impl->CreateStream(); }

void GstCustomDataReceiver::AddIntermidateElement(const std::string& elems) {
  m_impl->AddIntermidateElement(elems);
}

GstPipelineHandler* GstCustomDataReceiver::GetPipeline() { return m_impl; }
uint GstCustomDataReceiver::GetDataPort() { return m_impl->GetPort(); }

void GstCustomDataReceiver::SetVolume(float vol) { m_impl->SetVolume(vol); }
bool GstCustomDataReceiver::IsStream() { return m_impl->IsStream(); }

void GstCustomDataReceiver::Play() { m_impl->Play(); }
void GstCustomDataReceiver::Stop() { m_impl->Stop(); }
void GstCustomDataReceiver::Pause() { m_impl->Pause(); }
bool GstCustomDataReceiver::IsLoaded() { return m_impl->IsLoaded(); }
bool GstCustomDataReceiver::IsPlaying() { return m_impl->IsPlaying(); }
void GstCustomDataReceiver::Close() { m_impl->Close(); }

int GstCustomDataReceiver::GetFramesCount() { return m_impl->GetFramesCount(); }

bool GstCustomDataReceiver::GrabFrame(int i) { return m_impl->GrabFrame(i); }
...
int GstCustomDataReceiver::GetPort(int i) { return m_impl->GetPort(i); }
